...
router.post("/add", auth(["owner"]), (req, res) => {
  const { name, latitude, longitude, price, type, initial_slots = 1 } = req.body;
  ...
  if (initial_slots < 1 || initial_slots > 100) {
    return res.status(400).json({ message: "Initial slots must be between 1 and 100" });
  }
  ...
  db.query(
    `INSERT INTO parking_spots 
     ...
     VALUES (?,?,?,?,?,?,?,?,?, NOW())`,
    [req.user.id, name, latitude, longitude, price, type, initial_slots, initial_slots, initial_slots],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
router.delete("/:id", auth(["owner"]), (req, res) => {
  ...
  db.query(checkQuery, [id], (err, result) => {
    ...
    db.query("DELETE FROM availability WHERE parking_id=?", [id], (err2) => {
      ...
      db.query("DELETE FROM parking_slots WHERE parking_spot_id=?", [id], (err3) => {
        if (err3) return res.status(500).json({ error: err3.message });

        // Delete parking spot
        db.query("DELETE FROM parking_spots WHERE id=?", [id], (err4) => {
          if (err4) return res.status(500).json({ error: err4.message });
          res.json({ message: "Parking spot and all its slots removed successfully" });
        });
      });
    });
  });
