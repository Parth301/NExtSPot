...
const validateRating = (rating) =>
  rating && Number.isInteger(rating) && rating >= MIN_RATING && rating <= MAX_RATING;
...
const validateReviewData = (req, res, next) => {
  ...
  const optionalRatings = [cleanliness_rating, safety_rating, accessibility_rating];
...
router.post("/submit", auth(["user"]), validateReviewData, (req, res) => {
  ...
  if (!booking_id) {
    return res.status(400).json({
      message: "Booking ID is required. You can only review after completing a reservation.",
    });
  }
    ...
    (err, bookingResult) => {
      ...
      if (!bookingResult.length) {
        return res.status(403).json({
          message: "Invalid booking. You can only review expired reservations that have ended.",
        });
      }
      ...
      db.query(
        ...
        (err2, existingReview) => {
          if (err2) {
            ...
            return res.status(500).json({ message: "Error checking existing review" });
          }
...
router.post("/:reviewId/vote", auth(["user"]), (req, res) => {
  ...
  if (typeof is_helpful !== "boolean") {
    return res.status(400).json({
      message: "is_helpful must be a boolean (true for helpful, false for not helpful)",
    });
  }
      ...
      db.query(bookingQuery, [req.user.id, parkingId], (err2, bookings) => {
        ...
        if (!bookings.length) {
          return res.json({
            ...
            message: "You need to complete a reservation before reviewing this spot",
          });
        }
...
// Get all reviews for owner's parking spots
router.get("/owner/reviews", auth(["owner"]), (req, res) => {
  ...
  db.query(query, [req.user.id, limit, offset], (err, reviews) => {
    if (err) {
...
router.post("/:reviewId/respond", auth(["owner"]), (req, res) => {
  ...
  if (response_text.length > MAX_RESPONSE_LENGTH) {
    return res
      .status(400)
      .json({ message: `Response must not exceed ${MAX_RESPONSE_LENGTH} characters` });
  }
  ...
  db.query(checkQuery, [reviewId], (err, result) => {
    ...
    db.query(insertQuery, [reviewId, req.user.id, response_text.trim()], (err2) => {
      if (err2) {
